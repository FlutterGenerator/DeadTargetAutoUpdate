...
const std::string expectedTitle = "BY HunMod HACK";
...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Dead Target Auto Update 64bit 32bit </b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:YELLOW;\">Modded by tojik_proof_93</p> | "
                                     "Modded by HunMod</p>"
                                     "</marquee></b>"));


    //Toast Example
    Toast(env,ctx,OBFUSCATE("MADE BY HunMod HACK @tojik_proof_93"),ToastLength::LENGTH_LONG);
